...
/**
 * Generate Copy + Captions queues one writer call. The helios-reels worker
 * claims it. The app does not call Claude itself.
 */
export async function POST(request: NextRequest) {
  ...
  try {
    const result = await queueCopyJob(body.post_idea_id, body.slate_id);
    if (!result.queued) {
