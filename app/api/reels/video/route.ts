...
/**
 * Generate video queues one clip. The helios-reels worker claims it. The app
 * does not call Claude or Higgsfield.
 */
export async function POST(request: NextRequest) {
  ...
  try {
    const result = await queueVideoJob(body.post_idea_id, body.slate_id);
    if (!result.queued) {
